    ...
    try:
        if "message" in data:
            ...
            # Handle Image Messages
                ...
                # Last item is usually the highest resolution
                photo_id = msg["photo"][-1]["file_id"]
                handle_image(chat_id, photo_id)
                
    except Exception as e:
        # Print the error and the full traceback to your logs (Render/Railway)
        print(f"[ERROR] {e}")
        print(traceback.format_exc())
        
        # If we caught the chat_id before the crash, notify the user
        if chat_id:
            try:
                send_message(chat_id, "⚠️ Maaf, berlaku ralat teknikal. Sila cuba lagi.\n\nSorry, a technical error occurred. Please try again.")
            except:
                pass
...
def send_message(chat_id, text):
    ...
    # Telegram uses the token directly in the URL
    url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
    
    payload = {
        "chat_id": chat_id,
        "text": text,
        "parse_mode": "Markdown"  # This allows *bold* and _italic_ text
    }
    
    # We use json=payload to send as application/json
    try:
        response = requests.post(url, json=payload)
        response.raise_for_status() # Optional: raises error for bad status codes
    except Exception as e:
        print(f"[ERROR] Failed to send Telegram message: {e}")

if __name__ == "__main__":
    # For Telegram bots on Railway/Render, we use the PORT env variable
...
def handle_text(chat_id, text):
    ...
    if text_upper in ["LANGKAU", "SKIP"]:
        ...
        if current_state == "content_generate":
            ...
            user_ref.update({"state": "menu"})
            if lang == "bm":
                send_message(chat_id, "⏭️ Pengesahan SSM dilangkau (7 mata, bukan 10).\n\n⏳ Sedang mengira skor kredit awak...")
            else:
                send_message(chat_id, "⏭️ SSM verification skipped (7 pts instead of 10).\n\n⏳ Calculating your credit score...")
            generate_credit_score(chat_id, user_ref)
            return
        elif current_state == "await_ssm_cert":
            user_ref.update({"state": "menu"})
            if lang == "bm":
                send_message(chat_id, "⏭️ Pengesahan SSM dilangkau.\n\nTaip *MENU* untuk kembali")
            else:
                send_message(chat_id, "⏭️ SSM verification skipped.\n\nType *MENU* to go back")
            return
            ...
            if lang == "bm":
                send_message(chat_id, f"⏭️ Pengesahan bank dilangkau (7 mata, bukan 10).\n\nSoalan 3️⃣: {cc['reg_question_bm']}\n_(Balas: Ya / Tidak)_")
            else:
                send_message(chat_id, f"⏭️ Bank verification skipped (7 pts instead of 10).\n\nQuestion 3️⃣: {cc['reg_question_en']}\n_(Reply: Yes / No)_")
            return
        elif current_state in ("await_bank_doc_then_score", "await_bank_doc"):
            user_ref.update({"state": "menu"})
            if lang == "bm":
                send_message(chat_id, "⏭️ Pengesahan bank dilangkau (7 mata, bukan 10).\n\nTaip *MENU* untuk kembali")
            else:
                send_message(chat_id, "⏭️ Bank verification skipped (7 pts instead of 10).\n\nType *MENU* to go back")
            return
        else:
            if lang == "bm":
                send_message(chat_id, "Tiada tindakan untuk dilangkau.\n\nTaip *MENU* untuk kembali.")
            else:
                send_message(chat_id, "Nothing to skip.\n\nType *MENU* to go back.")
            return
 ...
 # KEMASKINI/UPDATE command — re-answer credit questions
    ...
    if state == "ask_consent":
        ...
        if text_upper in country_map:
            ...
            if lang == "bm":
                send_message(chat_id, "Sila pilih 1-3:\n1️⃣ 🇲🇾 Malaysia\n2️⃣ 🇮🇩 Indonesia\n3️⃣ 🇵🇭 Philippines")
            else:
                send_message(chat_id, "Please choose 1-3:\n1️⃣ 🇲🇾 Malaysia\n2️⃣ 🇮🇩 Indonesia\n3️⃣ 🇵🇭 Philippines")
...
"""
        ...
        if text_upper in ["HAI", "HI", "HELLO", "START"]:
            ...
            if text_upper in ["1","2","3","4","5","6","7","8","9","MENU","PROFIL","PROFILE","SIJIL","CERTIFICATE","PINJAMAN","LOAN","RESET","DATA","BREAKDOWN","PECAHAN","RUJUK","REFER","KEMASKINI","UPDATE"]:
                handle_menu(chat_id, text, user_ref)
            else:
                smart_handle(chat_id, text, user_ref)

    elif state == "log_sale":
        handle_log_sale(chat_id, text, user_ref)

    elif state == "ai_chat":
        handle_ai_chat(chat_id, text, user_ref)

    elif state == "credit_confirm":
        if text_upper in ["YA", "YES", "Y", "SAMA", "BETUL", "CORRECT"]:
            user_ref.update({"state": "menu"})
            if lang == "bm":
                send_message(chat_id, "⏳ Sedang mengira skor kredit awak...")
            else:
                send_message(chat_id, "⏳ Calculating your credit score...")
            generate_credit_score(chat_id, user_ref)
            ...
            if lang == "bm":
                send_message(chat_id, "🔄 *Kemaskini Maklumat*\n\nSoalan 1️⃣: Adakah awak ada *akaun bank perniagaan*?\n_(Balas: Ya / Tidak)_")
            else:
                send_message(chat_id, "🔄 *Update Info*\n\nQuestion 1️⃣: Do you have a *business bank account*?\n_(Reply: Yes / No)_")
        elif text_upper == "MENU":
            user_ref.update({"state": "menu"})
            handle_menu(chat_id, "MENU", user_ref)
        else:
            if lang == "bm":
                send_message(chat_id, "Sila balas *YA* atau *TIDAK*\n\nYA → Jana skor terus\nTIDAK → Kemaskini maklumat\nMENU → Kembali")
            else:
                send_message(chat_id, "Please reply *YES* or *NO*\n\nYES → Generate score now\nNO → Update info\nMENU → Go back")
        ...
        if text_upper.lower() in YES_ANSWERS:
            ...
            if lang == "bm":
                send_message(chat_id, "⚠️ Sila balas *Ya* atau *Tidak* sahaja.\n\nSoalan 1️⃣: Adakah awak ada *akaun bank perniagaan*?\n_(Balas: Ya / Tidak)_")
            else:
                send_message(chat_id, "⚠️ Please reply *Yes* or *No* only.\n\nQuestion 1️⃣: Do you have a *business bank account*?\n_(Reply: Yes / No)_")
            return
        if lang == "bm":
            send_message(chat_id, f"✅ Dikemaskini!\n\nSoalan 2️⃣: {cc['reg_question_bm']}\n_(Balas: Ya / Tidak)_")
        else:
        ...
        if text_upper.lower() in YES_ANSWERS:
            ...
            if lang == "bm":
                send_message(chat_id, f"⚠️ Sila balas *Ya* atau *Tidak* sahaja.\n\n{cc['reg_question_bm']}\n_(Balas: Ya / Tidak)_")
            else:
                send_message(chat_id, f"⚠️ Please reply *Yes* or *No* only.\n\n{cc['reg_question_en']}\n_(Reply: Yes / No)_")
   
    elif state == "credit_q1":
        user_ref.update({"biz_age": text, "state": "credit_q2"})
        if lang == "bm":
            send_message(chat_id, f"✅ Noted!\n\nSoalan 2️⃣: Adakah awak ada *akaun bank perniagaan*?\n_(Balas: Ya / Tidak)_")
        else:
        ...
        handle_content_menu(chat_id, text, user_ref)

    elif state == "content_generate":
        handle_content_generate(chat_id, text, user_ref)
...
# ─────────────────────────────────────
def smart_handle(chat_id, text, user_ref):
    user_data = user_ref.get().to_dict()
    lang = user_data.get("language", "bm")
    cur = get_currency(user_data)
    text_upper = text.upper().strip()
    ...
    if text_upper in ["KEMASKINI", "UPDATE"]:
        ...
        cc_temp = get_country(user_data)
        if lang == "bm":
            send_message(chat_id, "🔄 *Kemaskini Maklumat Kredit*\n\nSoalan 1️⃣: Berapa lama perniagaan awak dah beroperasi?\n_(Contoh: 3 bulan, 1 tahun, 5 tahun)_")
        else:
            send_message(chat_id, "🔄 *Update Credit Info*\n\nQuestion 1️⃣: How long has your business been operating?\n_(Example: 3 months, 1 year, 5 years)_")
        return
    if text_upper == "RESET":
        user_ref.delete()
        send_message(chat_id, "🔄 Profil dipadam. Taip HAI untuk mula semula." if lang=="bm" else "🔄 Profile deleted. Type HI to start again.")
        return
...
"""        
        ...
        if lang == "bm":
            send_message(chat_id, f"✅ *Perbelanjaan direkod!*\n\n💸 Jumlah: {cur}{expense['amount']}\n📦 Item: {expense['item']}\n\nTaip *MENU* untuk pilihan lain.")
        else:
            send_message(chat_id, f"✅ *Expense recorded!*\n\n💸 Amount: {cur}{expense['amount']}\n📦 Item: {expense['item']}\n\nType *MENU* for other options.")

    elif intent == "check_score":
        show_certificate(chat_id, user_ref)

    elif intent == "check_summary":
        show_sales_summary(chat_id, user_ref)
...
"""
        ...
        if lang == "bm":
            send_message(chat_id, f"🤖 *AI Penasihat:*\n\n{ai_response.text}\n\n_(Tanya lagi atau taip MENU)_")
        else:
            send_message(chat_id, f"🤖 *AI Advisor:*\n\n{ai_response.text}\n\n_(Ask more or type MENU)_")
...
"""
        ai_response = client.models.generate_content(model=MODEL, contents=ai_prompt)
        if lang == "bm":
            send_message(chat_id, f"🤖 {ai_response.text}\n\n_(Taip MENU untuk pilihan)_")
        else:
...
# MENU
# ─────────────────────────────────────
def handle_menu(chat_id, text, user_ref):
    ...
    if t_upper in ["MENU", "HI", "HAI", "START"]:
        ...
        show_sales_forecast(chat_id, user_ref)
    elif t_upper == "7":
        show_supply_chain_tips(chat_id, user_ref)
    elif t_upper == "8":
        show_export_guide(chat_id, user_ref)
    else:
        if lang == "bm":
            send_message(chat_id, "Taip *MENU* untuk lihat pilihan awak 😊")
        else:
            send_message(chat_id, "Type *MENU* to see your options 😊")

# ─────────────────────────────────────
# LOG SALE
# ─────────────────────────────────────
def handle_log_sale(chat_id, text, user_ref):
    user_data = user_ref.get().to_dict()
    lang = user_data.get("language", "bm")
    cur = get_currency(user_data)
    ...
    if lang == "bm":
        send_message(chat_id, f"✅ *Jualan direkod!*\n\n💵 Jumlah: {cur}{sale['amount']}\n📦 Item: {sale['item']}\n\nData ini disimpan untuk profil kredit awak 📊\nTaip *MENU* untuk kembali")
    else:
        send_message(chat_id, f"✅ *Sale recorded!*\n\n💵 Amount: {cur}{sale['amount']}\n📦 Item: {sale['item']}\n\nThis data is saved for your credit profile 📊\nType *MENU* to go back")

# ─────────────────────────────────────
# AI CHAT
# ─────────────────────────────────────
def handle_ai_chat(chat_id, text, user_ref):
    user_data = user_ref.get().to_dict()
    lang = user_data.get("language", "bm")
...
# ─────────────────────────────────────
def show_stored_data(chat_id, user_ref):
    user_data = user_ref.get().to_dict()
    lang = user_data.get("language", "bm")
    sales = user_data.get("sales", [])
...
def generate_credit_score(chat_id, user_ref):
    user_data = user_ref.get().to_dict()
    sales = user_data.get("sales", [])
    total = sum(s.get("amount", 0) for s in sales)
    count = len(sales)
...
# SALES SUMMARY
# ─────────────────────────────────────
def show_sales_summary(chat_id, user_ref):
    user_data = user_ref.get().to_dict()
    sales = user_data.get("sales", [])
    expenses = user_data.get("expenses", [])
    lang = user_data.get("language", "bm")
...
# EXPORT PROFILE
# ─────────────────────────────────────
def show_profile(chat_id, user_ref):
    user_data = user_ref.get().to_dict()
    lang = user_data.get("language", "bm")
    cc = get_country(user_data)
    cur = cc["currency"]
...
# CREDIT CERTIFICATE
# ─────────────────────────────────────
def show_certificate(chat_id, user_ref):
    user_data = user_ref.get().to_dict()
    lang = user_data.get("language", "bm")
    score = user_data.get("credit_score", 0)
    ...
    if not score:
        if lang == "bm":
            send_message(chat_id, "⚠️ Awak belum jana skor kredit lagi.\nTaip *MENU* → pilih *2* untuk jana skor kredit awak dahulu.")
        else:
            send_message(chat_id, "⚠️ You haven't generated a credit score yet.\nType *MENU* → choose *2* to generate your credit score first.")
        return
    ...
    if lang == "bm":
        send_message(chat_id,
            ...
            f"🆔 ID: NC-{chat_id[-4:]}-{score_date.replace('-','')}\n\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
...
# ─────────────────────────────────────
def prompt_ssm_verification(chat_id, user_ref):
    user_data = user_ref.get().to_dict()
    lang = user_data.get("language", "bm")
    cc = get_country(user_data)
...
def handle_image(chat_id, file_id):
    """Integrated: Downloads from Telegram and runs BizBuddy Logic"""
    user_ref = db.collection("users").document(chat_id)
    ...
    if lang == "bm":
        send_message(chat_id, "📸 Gambar diterima! Sedang menganalisis... ⏳")
    else:
        send_message(chat_id, "📸 Image received! Analyzing... ⏳")
    ...
    try:
        ...
        error_msg = "Maaf, gagal memuat turun gambar." if lang == "bm" else "Sorry, failed to download the image."
        send_message(chat_id, error_msg)
        return

    # 3. Route based on current state (Same as WhatsApp logic)
    if state in ("await_ssm_cert", "await_ssm_cert_then_score"):
        handle_ssm_verification(chat_id, user_ref, user_data, img, after_score=(state == "await_ssm_cert_then_score"))
        return

    if state in ("await_bank_doc_then_reg", "await_bank_doc_then_score", "await_bank_doc"):
        handle_bank_verification(chat_id, user_ref, user_data, img, mode=state)
        return

    # 4. Triage Document Type using Gemini
    triage_prompt = """
    ...
    # 5. Process based on Triage
    if doc_type == "ssm_cert":
        handle_ssm_verification(chat_id, user_ref, user_data, img)
        return

    # 6. Default: Payment Extraction Flow
    prompt = """
    ...
    if data.get("is_payment") and data.get("amount", 0) > 0:
        ...
        currency = "RM" # Or get_currency(user_data)
        if lang == "bm":
            send_message(chat_id, f"✅ *Bayaran direkod!*\n\n💵 Jumlah: {currency}{data['amount']}\n📦 Item: {data.get('item', 'Jualan')}\n\nTaip *MENU* untuk kembali")
        else:
            send_message(chat_id, f"✅ *Payment recorded!*\n\n💵 Amount: {currency}{data['amount']}\n📦 Item: {data.get('item', 'Sale')}\n\nType *MENU* to go back")
    else:
        # User Feedback for unknown images
        msg_bm = "Saya tidak dapat mengesan bayaran. Jika ini sijil SSM, taip *VERIFY*."
        msg_en = "I couldn't detect a payment. If this is an SSM cert, type *VERIFY*."
        send_message(chat_id, msg_bm if lang == "bm" else msg_en)


# ─────────────────────────────────────
# SSM / BUSINESS CERT VERIFICATION
# ─────────────────────────────────────
def handle_ssm_verification(chat_id, user_ref, user_data, img, after_score=False):
...
# ─────────────────────────────────────
def handle_bank_verification(chat_id, user_ref, user_data, img, mode="await_bank_doc_then_reg"):
...
{"is_bank_document": true, "account_holder_name": "registered name", "bank_name": "wallet name (TNG/GoPay/GCash/Maya/Dana)", "account_number_last4": "last 4 of phone/account or null", "business_name_on_account": null, "doc_type": "e_wallet"}''',
    ...
    if not bank_data.get("is_bank_document"):
        if lang == "bm":
            send_message(chat_id, "❌ Gagal membaca dokumen. Sila cuba hantar gambar yang lebih jelas.\n\nTaip *LANGKAU* untuk langkau")
        else:
            send_message(chat_id, "❌ Failed to read document. Please try a clearer image.\n\nType *SKIP* to skip")
        return
    ...
    if mode == "await_bank_doc_then_reg":
        ...
        user_ref.update({"state": "menu"})
        if lang == "bm":
            send_message(chat_id, "Taip *SKOR* untuk lihat skor kredit terbaru awak\nTaip *MENU* untuk kembali")
        else:
            send_message(chat_id, "Type *SCORE* to see your updated credit score\nType *MENU* to go back")

# ─────────────────────────────────────
# LOAN READINESS CHECKLIST
# ─────────────────────────────────────
def show_loan_checklist(chat_id, user_ref):
    user_data = user_ref.get().to_dict()
    lang = user_data.get("language", "bm")
    cc = get_country(user_data)
    cur = cc["currency"]
...
# CONTENT MENU
# ─────────────────────────────────────
def handle_content_menu(chat_id, text, user_ref):
    user_data = user_ref.get().to_dict()
    lang = user_data.get("language", "bm")
    t_upper = text.upper().strip()
    ...
    if t_upper in content_types:
        ...
        if lang == "bm":
            send_message(chat_id, "Sila pilih 1-5 atau taip *MENU* untuk kembali.")
        else:
            send_message(chat_id, "Please choose 1-5 or type *MENU* to go back.")

# ─────────────────────────────────────
# CONTENT GENERATOR
# ─────────────────────────────────────
def handle_content_generate(chat_id, text, user_ref):
    user_data = user_ref.get().to_dict()
    lang = user_data.get("language", "bm")
...
"""
    ...
    if lang == "bm":
        send_message(chat_id, "⏳ Sedang menjana kandungan untuk awak...")
    else:
        send_message(chat_id, "⏳ Generating content for you...")
...
# ─────────────────────────────────────
def show_score_breakdown(chat_id, user_ref):
    user_data = user_ref.get().to_dict()
    lang = user_data.get("language", "bm")
    cc = get_country(user_data)
    score = user_data.get("credit_score", 0)
...
# ─────────────────────────────────────
def show_loan_referral(chat_id, user_ref):
    user_data = user_ref.get().to_dict()
    lang = user_data.get("language", "bm")
    cc = get_country(user_data)
    cur = cc["currency"]
    ...
    txn_count = len(sales)
    score_date = user_data.get("score_date", "-")
    cert_id = f"NC-{chat_id[-4:]}-{score_date.replace('-', '')}"
    loan = cc["loan_program"]
...
# ─────────────────────────────────────
def show_sales_forecast(chat_id, user_ref):
    user_data = user_ref.get().to_dict()
    lang = user_data.get("language", "bm")
    cc = get_country(user_data)
    cur = cc["currency"]
    ...
    if count < 3:
        if lang == "bm":
            send_message(chat_id, "⚠️ *Perlukan sekurang-kurangnya 3 rekod jualan* untuk ramalan AI.\n\nRekod lebih banyak jualan dahulu, kemudian cuba lagi!\nTaip *MENU* untuk kembali")
        else:
            send_message(chat_id, "⚠️ *Need at least 3 sales records* for AI forecast.\n\nRecord more sales first, then try again!\nType *MENU* to go back")
        return

    if lang == "bm":
        send_message(chat_id, "📈 Sedang menganalisis corak jualan awak...")
    else:
        send_message(chat_id, "📈 Analyzing your sales patterns...")
...
# ─────────────────────────────────────
def show_supply_chain_tips(chat_id, user_ref):
    user_data = user_ref.get().to_dict()
    lang = user_data.get("language", "bm")
    cc = get_country(user_data)
    cur = cc["currency"]
    ...
    if lang == "bm":
        send_message(chat_id, "📦 Sedang menganalisis rantaian bekalan awak...")
    else:
        send_message(chat_id, "📦 Analyzing your supply chain...")
...
# ─────────────────────────────────────
def show_export_guide(chat_id, user_ref):
    user_data = user_ref.get().to_dict()
    lang = user_data.get("language", "bm")
    cc = get_country(user_data)
    cur = cc["currency"]
    ...
    if lang == "bm":
        send_message(chat_id, "🌏 Sedang menganalisis peluang eksport ASEAN...")
    else:
        send_message(chat_id, "🌏 Analyzing ASEAN export opportunities...")
